...
def draw_parameters(parameters, objects, maxWidths):
    ...
    for parameter in parameters:
        ...
        parameterPart = Fore.RED + ('%' + str(maxWidths['PARAMETER']) + 's') % parameter + Style.RESET_ALL
        coefficientPart = Fore.BLUE + ('%' + str(maxWidths['COEFFICIENT']) + 's') % parameters[parameter]['COEFFICIENT'] + Style.RESET_ALL
        objectsPart = ''
...
def print_total (header, objects, parameters, maxWidths):
    ...
    for obj in objTotal:
        if maxValue == obj:
            totalPart += Fore.BLUE + Back.MAGENTA
    ...
    print(totalPart)
    print(header_line)
    print('Best object has ' + Fore.BLUE + Back.MAGENTA + 'Magenta color (' + maxValue + ')' + Style.RESET_ALL)
...
print('\nStart data from file "' + sys.argv[1] + '"')
